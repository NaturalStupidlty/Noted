...
class NoteAgent:
	...
	such as creating, updating, or searching for notes.
	"""

	# Define instructions as a class constant
	INSTRUCTIONS = (
	...
	def _locate_note_id_for_update(self, note_text: str, note_id: any, search_query: any) -> any:
		"""
		Determines the note ID for updating.
		...
		Raises:
			HTTPException: If no note is found to update.
		"""
	...
	def handle_request(self, text: str) -> dict:
		...
		if action in ["plain_note", "create_note"]:
			...
			note_id = self._locate_note_id_for_update(note_text, provided_note_id, search_query)
